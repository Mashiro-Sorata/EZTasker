...
class Configuration:
    ...
    DCFG = {'SICON': '',     # Icon for static state
            'DICON': '',     # Icon for running state
            ...
            'SOFTWARE': '',     # The path where the program or script is located
            'METHOD': 'THREADING',    # 'THREADING' for python, 'PROCESS' for executable file
            'AUTORUN': False,
...
class MyTaskBarIcon(wx.adv.TaskBarIcon): 
    ...
    ID_OPEN = wx.NewId()  # 菜单选项“显示页面”的ID
    ID_CLOSE = wx.NewId() # 菜单选项“关闭程序”的ID
    ID_LOG = wx.NewId()     #菜单选项“日志”的ID

    def __init__(self):
        self.cfg = Configuration()
        self.DICT = DICT[self.cfg.get('LANG')]
        self.DICON = DICON.GetIcon()
        self.SICON = SICON.GetIcon()
        wx.adv.TaskBarIcon.__init__(self)
        self.__set_icon('SICON') # 设置图标和标题
        ...
        self.Bind(wx.EVT_MENU, self.onOpen, id=self.ID_OPEN)  # 绑定“显示页面”选项的点击事件
    ...
    def __set_icon(self, state):
        if os.path.isfile(self.cfg.get(state)):
            ...
            self.SetIcon(self.DICON if state=='DICON' else self.SICON, self.cfg.get('TITLE'))


    # “关于”选项的事件处理器
    def onAbout(self, event):
        #wx.MessageBox(self.cfg.get('ABOUT'), self.DICT['About'])
...
class MyFrame(wx.Frame):
    def __init__(self):
        ...
        self.taskbar = MyTaskBarIcon()     #显示系统托盘图标
